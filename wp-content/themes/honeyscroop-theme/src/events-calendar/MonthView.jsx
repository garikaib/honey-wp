...
const MonthView = ({ events, currentDate, onNavigate, onEventClick }) => {
    ...
    return (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden animate-fade-in-up">
            {/* Calendar Header */}
            <div className="flex items-center justify-between p-6 border-b border-gray-100 bg-honey-50/30">
                <button
                    onClick={() => onNavigate('prev')}
                    className="p-2 hover:bg-honey-100 rounded-full transition-colors text-honey-800"
                ...
                <h3 className="text-2xl font-serif font-bold text-gray-800">
                    {monthNames[currentMonth]} <span className="text-honey-600">{currentYear}</span>
                </h3>
            ...
            <div className="grid grid-cols-7 auto-rows-fr bg-gray-100 gap-px border-b border-gray-100">
                {calendarDays.map((cell) => {
                    if (cell.type === 'padding') {
                        return <div key={cell.id} className="bg-gray-50/30 min-h-[120px]" />;
                    }
                    ...
                    return (
                        <div key={cell.id} className={`bg-white min-h-[120px] p-2 hover:bg-honey-50/10 transition-colors relative group ${isToday ? 'bg-honey-50/30' : ''}`}>
                            <span className={`inline-block w-7 h-7 text-center leading-7 rounded-full text-sm font-medium mb-1 ${isToday ? 'bg-honey-500 text-white' : 'text-gray-500'}`}>
                                {cell.day}
                            </span>
                            ...
                            <div className="space-y-1">
                                {cell.events.map(event => (
                                    <div
                                        ...
                                        className="text-xs p-1.5 rounded bg-honey-100/50 text-honey-900 border-l-2 border-honey-400 cursor-pointer hover:bg-honey-200 transition-colors truncate"
                                        title={event.title.rendered}
                                    >
                                        <div className="font-bold truncate" dangerouslySetInnerHTML={{ __html: event.title.rendered }} />
                                        <div className="text-[10px] text-honey-700 flex items-center gap-1">
