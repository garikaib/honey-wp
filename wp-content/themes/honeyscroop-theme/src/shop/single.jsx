            ...
            <div className="container mx-auto px-4 py-12 md:py-20">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-20">
                    ...
                    <div className="relative">
                        <div className="rounded-xl overflow-hidden shadow-xl bg-gray-50 dark:bg-white/5 border border-honey-100 dark:border-white/10 transition-colors">
                    ...
                    {/* Details Side */}
                    <div>
                        <h1 className="text-4xl md:text-5xl font-serif font-bold text-honey-900 dark:text-honey-50 mb-4 transition-colors" dangerouslySetInnerHTML={{ __html: product.title.rendered }} />

                        <div className="text-2xl font-medium text-honey-600 dark:text-honey-400 mb-6 transition-colors">
                            {formatPrice(price / 100)}
                        </div>

                        <div className="prose prose-lg text-gray-600 dark:text-gray-300 mb-8 transition-colors" dangerouslySetInnerHTML={{ __html: product.content.rendered }} />
                        ...
                        {!isOutOfStock ? (
                            <div className="flex flex-col sm:flex-row gap-4 items-center mb-6">
                                {/* Qty Selector */}
                                <div className="flex items-center border border-gray-300 dark:border-white/20 rounded-xl overflow-hidden shadow-sm transition-colors">
                                    <button
                                        className="px-5 py-3 bg-gray-50 dark:bg-white/5 hover:bg-gray-100 dark:hover:bg-white/10 transition text-gray-600 dark:text-gray-200"
                                        onClick={() => setQty(q => Math.max(1, q - 1))}
                                    >-</button>
                                    <div className="px-5 py-3 min-w-[3.5rem] text-center font-bold bg-white dark:bg-white/5 text-lg text-gray-900 dark:text-white transition-colors">{qty}</div>
                                    <button
                                        className="px-5 py-3 bg-gray-50 dark:bg-white/5 hover:bg-gray-100 dark:hover:bg-white/10 transition text-gray-600 dark:text-gray-200"
                        ...
                        {!isOutOfStock && (
                            <div className="flex flex-col gap-3 mt-8">
                                ...
                                {/* Secondary: WhatsApp Concierge */}
                                <div className="text-center pt-2">
                                    <p className="text-xs text-gray-400 dark:text-gray-500 mb-2 uppercase tracking-widest font-bold">Need a custom order?</p>
                                    <a
